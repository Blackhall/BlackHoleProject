...
import com.example.developermode.blackhole.Effect.CollisionLineParticleEffect;
    ...
    public void Update() {
            ...
            if(plnt.m_iHP <= 0) {
                ...
                for(int j = m_PlanetList.size() - 1; j >= 0; j--) {
                    ...
                    if(CollisionTool.IsCollided(
                        ...
                        expPlnt.m_f2Velocity.x += f2ExplosionVelocity.x * plnt.m_fRadius * 1.4f * fInfluence;
                        expPlnt.m_f2Velocity.y += f2ExplosionVelocity.y * plnt.m_fRadius * 1.4f * fInfluence;
                ...
                for(int r = 0; r < 4; r++) {
                    m_EffectManager.RegisterEffectObj(new CollisionLineParticleEffect(
                            plnt.GetX(),
                            plnt.GetY(),
                            (float)Math.cos(Math.toRadians(360.0f/4.0f * r)), (float)Math.sin(Math.toRadians(360.0f/4.0f * r)),
                            plnt.m_fMass / 2.5f,
                            1.4f));
                }

                m_EffectManager.RegisterEffectObj(new ExplosionFlashEffect(plnt.GetX(), plnt.GetY(), plnt.m_fRadius, plnt.m_fRadius / 520.0f + 0.4f));
                AppManager.getInstance().m_VB.vibrate((int)(plnt.m_fRadius / 820.0f * 1000.0f));
        ...
        for(int i = 0; i < m_PlanetList.size(); i++) {
            for(int j = i + 1; j < m_PlanetList.size(); j++) {
                if(CollisionTool.IsCollided(m_PlanetList.get(i).GetX(), m_PlanetList.get(i).GetY(), m_PlanetList.get(i).m_fRadius,
                        m_PlanetList.get(j).GetX(), m_PlanetList.get(j).GetY(), m_PlanetList.get(j).m_fRadius)) {
                    Float2 f2J = new Float2();
                    CollisionTool.ImpurseBasedCollisionResolution(m_PlanetList.get(i), m_PlanetList.get(j), f2J);
                    Float2 fDirToJ = new Float2(
                            m_PlanetList.get(j).GetX() - m_PlanetList.get(i).GetX(),
                            m_PlanetList.get(j).GetY() - m_PlanetList.get(i).GetY());
                    CollisionTool.Float2Normalize(fDirToJ, fDirToJ);

                    m_EffectManager.RegisterEffectObj(new CollisionLineParticleEffect(
                            fDirToJ.x * m_PlanetList.get(i).m_fRadius + m_PlanetList.get(i).GetX(),
                            fDirToJ.y * m_PlanetList.get(i).m_fRadius + m_PlanetList.get(i).GetY(),
                            fDirToJ.x, fDirToJ.y,
                            CollisionTool.GetDistance(f2J, new Float2(0.0f, 0.0f)) / 2600.0f,
                            CollisionTool.GetDistance(f2J, new Float2(0.0f, 0.0f)) / 6500.0f));
                }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch(event.getAction() & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_POINTER_DOWN:
                ...
                for(int c = 0; c < pointer_count; c++) {
                    bIsCollided = false;
                    int px = (int) event.getX(c);
                    int py = (int) event.getY(c);
                    ...
                    for(int i = m_PlanetList.size() - 1; i >= 0; i--) {
                        ...
                        if(CollisionTool.IsCollided(plnt, px, py)) {
                            ...
                            if(35.0f > fDistance/plnt.m_fRadius * 100.0f) {
                                AppManager.getInstance().m_VB.vibrate(50);
                                m_EffectManager.RegisterEffectObj(new TwinklingPlanetEffect(
                                        plnt,
                                        0.0f,
                                        Color.rgb(255, 150, 0),
                                ...
                                iDamage = (ClickDamage + ClickDamage * 65 / 100) + (ClickDamage + ClickDamage * 65 / 100) * randTool.nextInt(21) / 100;
                                m_DamageManager.RegisterDamageObj(new Damage(
                                        iDamage,
                                ...
                                AppManager.getInstance().m_VB.vibrate(35);
                                m_EffectManager.RegisterEffectObj(new TwinklingPlanetEffect(
                                        plnt,
                                        0.0f,
                                        Color.rgb(255, 255, 0),
                                ...
                                iDamage = (ClickDamage + ClickDamage * 45 / 100) + (ClickDamage + ClickDamage * 45 / 100) * randTool.nextInt(21) / 100;
                                m_DamageManager.RegisterDamageObj(new Damage(
                                        iDamage,
                                        px + (randTool.nextInt() % (10 + 10 + 1) - 10) * 3,
                                        py + (randTool.nextInt() % (10 + 10 + 1) - 10) * 3,
                                ...
                                AppManager.getInstance().m_VB.vibrate(20);
                                m_EffectManager.RegisterEffectObj(new TwinklingPlanetEffect(
                                        plnt,
                                        0.0f,
                                        Color.rgb(150, 255, 0),
                    ...
                    if(!bIsCollided) {
                        AppManager.getInstance().m_VB.vibrate(10);
                        m_EffectManager.RegisterEffectObj(new WaveEffect(
                                (float)px, (float)py,
                                50.0f, 250.0f,
                                Color.rgb(0, 50, 150),
