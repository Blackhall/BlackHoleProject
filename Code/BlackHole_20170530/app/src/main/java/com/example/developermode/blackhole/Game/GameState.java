            ...
            if(plnt.m_iHP <= 0) {
                ...
                for(int r = 0; r < 5; r++) {
                    m_EffectManager.RegisterEffectObj(new CollisionLineParticleEffect(
                            plnt.GetX(),
                            plnt.GetY(),
                            (float)Math.cos(Math.toRadians(360.0f/5.0f * r)), (float)Math.cos(Math.toRadians(360.0f/5.0f * r)),
                            plnt.m_fMass / 3.0f,
                            1.6f));
                }

                m_EffectManager.RegisterEffectObj(new ExplosionFlashEffect(plnt.GetX(), plnt.GetY(), plnt.m_fRadius, 0.5f));
            ...
            for(int j = i + 1; j < m_PlanetList.size(); j++) {
                if(CollisionTool.IsCollided(m_PlanetList.get(i).GetX(), m_PlanetList.get(i).GetY(), m_PlanetList.get(i).m_fRadius,
                    ...
                    m_EffectManager.RegisterEffectObj(new CollisionLineParticleEffect(
                            ...
                            fDirToJ.x, fDirToJ.y,
                            CollisionTool.GetDistance(f2J, new Float2(0.0f, 0.0f)) / 2400.0f,
                            2.0f));
                }
